...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    // Get Old Avatar Image
    const oldAvatarImageURL = req.user.avatar;
    console.log(oldAvatarImageURL);
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    // Sequence of Searches(Pipelines)
    const channel = await User.aggregate(
        ...
        },
        {
            $addFields: {
                subscriberCount: {
                    // As this is also a field so add $
                    $size: "$subscribers",
                },
    ...
    // Usually gets an Array
    console.log(channel);

    return res
        .status(200)
        .json(new ApiResponse(200, channel[0], "Channel Data Fetched"));
});
